...
#define RMI_RW_ATTR (S_IRUGO | S_IWUSR)
//#define RMI_WO_ATTR S_IWUGO
#define RMI_WO_ATTR S_IWUSR
...
struct rmi_device_platform_data {
	...
	//Begin:added by hushaowu
	int (*gpio_config)(int mode);
    int (*set_power) (int on, const char* name);
    int (*tp_gpio_request) (int on);
	int (*tp_gpio_get_value)(void);
	int (*tp_gpio_reset)(const char *name);
    //End:added by hushaowu
	int reset_pin;
	int reset_delay_ms;
...
{
    #ifdef DEBUG_PRINT
    dev_warn(&d->dev, "[SYNA] Attempt to read 0x%X.\n", addr);
    #endif
...
{
    #ifdef DEBUG_PRINT
    dev_warn(&d->dev, "[SYNA] Attempt to write 0x%X characters to 0x%X.\n", addr, data);
    #endif
...
{
    #ifdef DEBUG_PRINT
    dev_warn(&d->dev, "[SYNA] Attempt to write 0x%X characters to %s.\n", addr, buf);
    #endif
