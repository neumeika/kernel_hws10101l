...
int iomux_block_set_config(char* block_name, enum iomux_func newmode)
{
...
EXPORT_SYMBOL(iomux_block_set_config);

int iomux_block_set_work_mode(char* block_name)
{
	return iomux_block_set_config(block_name,NORMAL);
}
EXPORT_SYMBOL(iomux_block_set_work_mode);

int iomux_block_set_lowpower_mode(char* block_name)
{
	return iomux_block_set_config(block_name,LOWPOWER);
}
EXPORT_SYMBOL(iomux_block_set_lowpower_mode);

int iomux_block_set_gpio_mode(char* block_name)
{
	return iomux_block_set_config(block_name,GPIO);
}
EXPORT_SYMBOL(iomux_block_set_gpio_mode);

int iomux_blocks_set_configs(char** block_name_array, unsigned int array_size, enum iomux_func newmode)
{
    char* pblock_name = *block_name_array;
    unsigned int icount = 0;
	struct iomux_block *block_tmp = NULL;
	struct block_config *config_tmp = NULL;
	int ret = 0;

    for(icount = 0; icount < array_size; icount++,pblock_name++)
    {
    	block_tmp = iomux_get_block(pblock_name);
    	if (!block_tmp) {
    		pr_err("%s: iomux_get_block(%s) error.\n",
    			__func__, pblock_name);
            ret = -INVALID;
    		break;
    	}

    	config_tmp = iomux_get_blockconfig(pblock_name);
    	if (!config_tmp) {
    		pr_err("%s: iomux_get_blockconfig(%s) error.\n",
    			__func__, pblock_name);
    		ret = -INVALID;
...
EXPORT_SYMBOL(iomux_blocks_set_configs);

int iomux_blocks_set_work_mode(char** block_name_array, unsigned int array_size)
{
	return iomux_blocks_set_configs(block_name_array,array_size,NORMAL);
}
EXPORT_SYMBOL(iomux_blocks_set_work_mode);

int iomux_blocks_set_lowpower_mode(char** block_name_array, unsigned int array_size)
{
	return iomux_blocks_set_configs(block_name_array,array_size,LOWPOWER);
}
EXPORT_SYMBOL(iomux_blocks_set_lowpower_mode);

int iomux_blocks_set_gpio_mode(char** block_name_array, unsigned int array_size)
{
	return iomux_blocks_set_configs(block_name_array,array_size,GPIO);
}
EXPORT_SYMBOL(iomux_blocks_set_gpio_mode);
