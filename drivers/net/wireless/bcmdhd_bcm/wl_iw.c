        ...
        char *extra)
{
	...
	if ((iw->pub->busstate == DHD_BUS_DOWN) || iw->pub->hang_was_sent) {
		printk(KERN_ERR "%s : bus is down. we have nothing to do\n", __FUNCTION__ );
		return -EINVAL;
	}
...
wl_is_event_need_update(char *macaddr, int mac_event_type)
{
	...
	if(MAC_LIST_MAX != idx)
	{
		if (mac_event_type != mslist[idx].mac_event_type){
			...
			WL_SOFTAP((" TRUE mac_event_type=%d\n", mac_event_type));
			
			return TRUE;
		} else {

			WL_SOFTAP((" FALSE  mac_event_type=%d, mslist[idx].mac_event_type=%d\n", mac_event_type, mslist[idx].mac_event_type));
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_REASSOC_IND:
		...
		if  ( strstr(fw_path, "apsta") != NULL &&
			...
			WL_SOFTAP(("STA connect received E_STA_JOIN \n"));
