...
static irqreturn_t k3v2_gpio_key_irq_handler(int irq, void *dev_id)
{
	...
	case IRQ_GPIO(GPIO_17_1):
		schedule_delayed_work(&(gpio_key->gpio_keyup_work), msecs_to_jiffies(10));
		break;

	case IRQ_GPIO(GPIO_17_2):
		schedule_delayed_work(&(gpio_key->gpio_keydown_work), msecs_to_jiffies(10));
		break;
...
static int __devinit k3v2_gpio_key_probe(struct platform_device* pdev)
{
    ...
    #if S10_HWID_L3H(S10, S10101, A)
    struct s10_gpio_key_platdata *pdata = NULL;
    pdata = pdev->dev.platform_data;
    if(NULL == pdata)
    {
        printk("[gpiokey]platform data error!\n");
        goto err_platdata;
    }
    if(pdata->init)
    {
		...
		if(err)
		{
			printk("[gpiokey]set gpio block failed\n");
			goto set_gpio_block_fail;
		}
    }
    err = gpio_request(pdata->vol_up, "vol_up");
    if(err)
    {
        printk(KERN_ERR "[gpiokey]gpio_request key vol_up error!\n");
        goto err_request_gpio_vol_up;
    }
    gpio_direction_input(pdata->vol_up);

    err = gpio_request(pdata->vol_down, "vol_down");
    if(err)
    {
        printk(KERN_ERR "[gpiokey]gpio_request key vol_down error!\n");       
        goto err_request_gpio_vol_down;
    }
    gpio_direction_input(pdata->vol_down);
    #endif
...
err_alloc_input_device:
	kfree(gpio_key);
	gpio_key = NULL;
err_request_gpio_vol_down:
	gpio_free(pdata->vol_up);	
err_request_gpio_vol_up:
set_gpio_block_fail:
err_platdata:
...
static int __devexit k3v2_gpio_key_remove(struct platform_device* pdev)
{
    ...
    #if S10_HWID_L3H(S10, S10101, A)
    struct s10_gpio_key_platdata *pdata = pdev->dev.platform_data;
	int err = 0;
    #endif
    ...
    #if S10_HWID_L3H(S10, S10101, A)
    gpio_free(pdata->vol_up);
	gpio_free(pdata->vol_down);
	if(pdata && pdata->exit)
    {
       err = pdata->exit();
	   if(err)
	   {
	   		printk("[gpiokey]set gpio block exit failed\n");
	   }
    }
    #endif
...
static int k3v2_gpio_key_resume(struct platform_device *pdev)
{
    ...
    if(pdata && pdata->init)
    {
	   ...
	   if(err)
	   {
	   		printk("[gpiokey]set gpio block init failed\n");
	   }
    }
	#endif
