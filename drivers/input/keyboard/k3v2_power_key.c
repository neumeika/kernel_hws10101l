...
struct k3v2_power_key	*g_power_key = NULL;

static ssize_t k3v2_power_key_attr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct k3v2_power_key *power_key = NULL;
    power_key = g_power_key;
    if(power_key)
    {
...
static DEVICE_ATTR(state, S_IRUSR | S_IWUSR, NULL, k3v2_power_key_attr_store);
...
static struct attribute* k3v2_power_key_attributes[] = {
        &dev_attr_state.attr,
        NULL
};

static struct attribute_group k3v2_power_key_defattr_group = {
        .attrs = k3v2_power_key_attributes,
};
...
static int __devinit k3v2_power_key_probe(struct platform_device* pdev)
{
    ...
    err = sysfs_create_group(&(pdev->dev.kobj), &k3v2_power_key_defattr_group);
	if(err)
	{
		goto err_get_irq;
	}
