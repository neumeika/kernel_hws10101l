...
static int rmi_driver_suspend(struct device *dev)
{
	...
	data = rmi_get_driverdata(rmi_dev);
#ifdef TP_POWER_DOWN
    dev_err(&rmi_dev->dev, "Enter %s\n", __func__);
	struct rmi_device_platform_data *pdata;
	pdata = to_rmi_platform_data(rmi_dev);
    if (pdata->set_power) {
    retval = pdata->set_power(0, "synaptics");
         if (retval < 0) {
             dev_err(&rmi_dev->dev, "failed to set power off! retval = %d\n", retval);
         }
    }
    if (pdata->gpio_config) {
        retval = pdata->gpio_config(LOWPOWER);
	    if (retval < 0) {
            dev_err(&rmi_dev->dev, "failed to set gpio mode. error %d\n", retval);
        }
    }
...
static int rmi_driver_resume(struct device *dev)
{
    ...
    pdata = to_rmi_platform_data(rmi_dev);
    if (pdata->gpio_config) {
        retval = pdata->gpio_config(NORMAL);
        if (retval < 0) {
            dev_err(&rmi_dev->dev, "failed to set gpio mode. error %d\n", retval);
        }
    }
    ...
    if (pdata->set_power) {
    retval = pdata->set_power(1, "synaptics");
         if (retval < 0) {
             dev_err(&rmi_dev->dev, "failed to set power on! retval = %d\n", retval);
         }
    }
#else
...
extern struct input_dev *input_suspend;
...
static void rmi_driver_early_suspend(struct early_suspend *h)
{
    ...
    dev_err(&rmi_dev->dev, "Early suspend  --1.\n");
#ifdef TP_POWER_DOWN
    dev_err(&rmi_dev->dev, "Enter %s\n", __func__);
    struct rmi_device_platform_data *pdata;
    pdata = to_rmi_platform_data(rmi_dev);
	...
	dev_err(&rmi_dev->dev, "%s,instance_data->status =  %d\n", __func__,instance_data->status);
    dev_err(&rmi_dev->dev, "%s,instance_data->inflashprogmode =  %d\n", __func__,instance_data->inflashprogmode);
    
	if(instance_data->inflashprogmode){
        dev_err(&rmi_dev->dev, "%s,It's in program mode.It does not power off!\n", __func__);
        dev_err(&rmi_dev->dev, "Exit %s\n", __func__);
        return;
    }
	...
	if (data->suspended)
		goto exit;
    dev_err(&rmi_dev->dev, "Early suspend  --2.\n");
	if (data->pre_suspend) {
		retval = data->pre_suspend(data->pm_data);
		if (retval)
			goto exit;
	}
    dev_err(&rmi_dev->dev, "Early suspend  --3.\n");
    ...
    dev_err(&rmi_dev->dev, "Early suspend  --4.\n");
    ...
    dev_err(&rmi_dev->dev, "Early suspend  --5.\n");
	data->suspended = true;


exit:
	mutex_unlock(&data->suspend_mutex);
    printk("suspend:data->suspended = %d\n",data->suspended);
#endif
}
...
static void rmi_driver_late_resume(struct early_suspend *h)
{
	...
	dev_err(&rmi_dev->dev, "Late resume   ---1.\n");
#ifdef TP_POWER_DOWN
    dev_err(&rmi_dev->dev, "Enter %s\n", __func__);
    struct rmi_device_platform_data *pdata;
    pdata = to_rmi_platform_data(rmi_dev);
    input_mt_sync(input_suspend);
    input_sync(input_suspend);
    if (pdata->gpio_config) {
        retval = pdata->gpio_config(NORMAL);
        if (retval < 0) {
            dev_err(&rmi_dev->dev, "failed to set gpio mode. error %d\n", retval);
	...
	// Delay for read the interrupt flags to clear the attention assertion. >=30ms.
	mdelay(30);
	u8 status =0;
    /* Clear the attention assertion by reading the interrupt status register */		
	retval = rmi_read(rmi_fc->rmi_dev, rmi_fc->fd.data_base_addr + 1, &status);
    ...
    dev_err(&rmi_dev->dev, "%s,The INIT Pin is low,It's wrong.Then reset the chip.\n",__func__,retval);
	
	if (pdata->tp_gpio_reset) {
    retval = pdata->tp_gpio_reset("synaptics");
         if (retval < 0) {
             dev_err(&rmi_dev->dev, "failed to reset the chip! retval = %d\n", retval);
         }
         dev_err(&rmi_dev->dev, "%s,GPIO_CTP_INT = %d.\n",__func__,gpio_get_value(GPIO_CTP_INT));
    }
	...
	mutex_lock(&data->suspend_mutex);
    printk("data->suspended = %d\n",data->suspended);
	if (!data->suspended)
		goto exit;
    dev_err(&rmi_dev->dev, "Late resume   ---2.\n");
    ...
    dev_err(&rmi_dev->dev, "Late resume   ---3.\n");
   ...
   dev_err(&rmi_dev->dev, "Late resume   ---4.\n");
    ...
    dev_err(&rmi_dev->dev, "Late resume   ---5.\n");
