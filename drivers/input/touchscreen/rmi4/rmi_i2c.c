...
irqreturn_t rmi_i2c_irq_thread(int irq, void *p)
{
			      ...
			      int len)
{
    ...
    //dev_err(&client->dev,"%s: i2c_master_recv,retval = %d\n", __func__, retval);
	if (retval < 0){
		phys->info.rx_errs++;
        dev_err(&client->dev,"%s: i2c_master_recv,retval = %d\n", __func__, retval);
	}
...
#if 1   //syna
static struct rmi_device_platform_data *rmi_ts = NULL;
struct rmi_device_platform_data *rmi_i2c_get_platform_data(void)
{
	return rmi_ts;
}
#endif
				  ...
				  const struct i2c_device_id *id)
{
    ...
    //added by hushaowu begin
    if (pdata->gpio_config) {
         error = pdata->gpio_config(NORMAL);
         if (error < 0) {
    ...
    //added by hushaowu
     ...
     if (pdata->set_power) {
         error = pdata->set_power(1, "synaptics");
         if (error < 0) {
