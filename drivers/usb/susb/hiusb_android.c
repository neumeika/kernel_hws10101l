...
void notify_charger_type(unsigned long val)
{
...
EXPORT_SYMBOL_GPL(notify_charger_type);
/**
...
void enable_vbus_irq(void)
{
	...
	enable_irq(irq);	
	hiusblog("enable vbus irq done\n");
}
EXPORT_SYMBOL(enable_vbus_irq);

void disable_vbus_irq(void)
{
	int irq;
	irq = IRQ_VBUS_COMP_VBAT_RISING;
	disable_irq(irq);
	irq = IRQ_VBUS_COMP_VBAT_FALLING;
	disable_irq(irq);	
	hiusblog("disable vbus irq done\n");
}
EXPORT_SYMBOL(disable_vbus_irq);
#endif
...
static int setup_dvc_and_phy(void)
{
	...
	if(chip_id == CS_CHIP_ID) {
		writel(0x0, IO_ADDRESS(0xFC8030AC));
		printk("%s, config for CS...\n", __func__);
	} else if (chip_id == DI_CHIP_ID) {
		writel(0x1, IO_ADDRESS(0xFC8030AC));
		printk("%s, config for ES...\n", __func__);
	}
#endif
	...
	switch (usbphy_tune) {
		...
		case E_USBPHY_TUNE_U9510:
			...
			reg_value |= (0x6 << 17);
			break;
		case E_USBPHY_TUNE_S10:
//			reg_value = 0x1b480f90;
			reg_value &= (~(PICOPHY_SIDDQ));
			reg_value |= PERI_CTRL16_PICOPHY_TXPREEMPHASISTUNE;
			reg_value &= ~(0x1 << 9); /* usb1_phy_otgdisable */
			reg_value &= ~(0x3 << 10); /* usb1_phy_vbusvldextsel */
...
static void k3v2_otg_interrupt_work(struct work_struct *work)
{
	...
	case CHARGER_IN_INTR:
		...
		if (HIUSB_DEVICE == hiusb_info_p->hiusb_status) {
			...
			notify_charger_type(hiusb_info_p->charger_type);
#endif
...
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT) 
			if((!get_product_feature(PROD_FEATURE_DETECT_MHL_CHIP))&&(HIUSB_SUSPEND == hiusb_info_p->hiusb_status))
            {
				Sii9244_mhl_reset();
				SiiSwitchDisConnect();
            }
	...
	case CHARGER_OUT_INTR:
		...
		break;
	case ID_FALL_EVENT:
/*open usb out5v,the pmu sense can make a hiusb_in interrupt,so we disable the interrupt*/
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT) 
		disable_vbus_irq();
		open_usb_out5v();
#ifdef CONFIG_BOOST_5V
		request_5v_boost(1, BOOST_5V_USB);
#endif
#endif
		if ((HIUSB_OFF == hiusb_info_p->hiusb_status)
		    || (HIUSB_SUSPEND == hiusb_info_p->hiusb_status)) {
			...
			setup_dvc_and_phy();
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT)
			hiusb_info_p->charger_type = CHARGER_TYPE_USB_OTG_LINE;
			notify_charger_type(hiusb_info_p->charger_type);
#endif
			dwc_otg_disable_global_interrupts(otg_dev->core_if);
...
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT)	
#ifdef CONFIG_BOOST_5V
                    request_5v_boost(0, BOOST_5V_USB);
#endif
                    close_usb_out5v();
                    enable_vbus_irq();
#endif
		...
		break;
	case ID_RISE_EVENT:
/*close usb out5v,the pmu sense can make a hiusb_out interrupt,so we disable the interrupt*/
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT) 
		disable_vbus_irq();
#ifdef CONFIG_BOOST_5V
		request_5v_boost(0, BOOST_5V_USB);
...
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT)
			open_usb_ap5v();
			hiusb_info_p->charger_type = CHARGER_USB_OTG_LINE_REMOVED;
			notify_charger_type(hiusb_info_p->charger_type);			
#endif
			hiusb_info_p->hiusb_status = HIUSB_OFF;
...
int hiusb_probe_phase2(struct lm_device *_dev)
{
	...
	if (dwc_otg_is_host_mode(otg_dev->core_if)) {
		...
		/* host mode mhl can detect id fall,so clearup otg */
		usb_status = HOST_MODE;
		k3v2_otg_clearup();
		...
		k3v2_otg_clearup();
	} else if (USB_CABLE_CONNECTED == hiusb_detect_usb_cable()) {
		...
		if(CHARGER_TYPE_STANDARD == hiusb_info_p->charger_type)
		{
			wake_unlock(&hiusb_info_p->hiusb_wake_lock);
			hiusblog("hiusb wake unlock\n");
		}
	...
	hiusb_request_irq(hiusb_info_p->hiusb_lm_dev);
		...
		/*no mhl chip,insert host line and power on,detect it*/
		if(get_product_feature(PROD_FEATURE_DETECT_MHL_CHIP)&&(HOST_MODE == usb_status))
		{
			hiusblog("host mode,no mhl chip\n");
...
int hiusb_suspend(struct lm_device *_dev)
{
	...
	if (HIUSB_HOST == hiusb_info_p->hiusb_status) {
		...
		hiusblog("Suspend in host mode, HOST -> SUSPEND\n");
	}
...
int hiusb_resume(struct lm_device *_dev)
{
	...
	if (HIUSB_SUSPEND == hiusb_info_p->hiusb_status) {
		...
		if (dwc_otg_is_host_mode(otg_dev->core_if)) {
			...
			hiusblog("In host mode, SUSPEND -> HOST\n");
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT) 
			open_usb_out5v();
#endif
...
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT) 
int k3v2_otg_id_status_change(enum usb_port_event event)
{
	unsigned long flags;
	int ret = 0;
...
int k3v2_otg_id_status_change(enum usb_port_event event)
{
	...
	if (!hiusb_info_p) {
		printk("USB OTG is not initialized yet\n");
		return -EBUSY;
	}
	...
	if (ID_FALL == event)
		...
		hiusb_info_p->intr_flag = ID_RISE_EVENT;
	else if (USB_CONNECT == event)
	{
		hiusb_info_p->intr_flag = CHARGER_IN_INTR;
	}
	else if (USB_DISCONNECT == event)
		hiusb_info_p->intr_flag = CHARGER_OUT_INTR;
	else {
		hiusblog("illegal status: %d!\n", event);
		spin_unlock_irqrestore(&(hiusb_info_p->intr_flag_lock), flags);
		return -EINVAL;
	}
	hiusb_info_p->intr_jiffies = jiffies;

	hiusblog("event: %d, intr_flag: %d\n", event, hiusb_info_p->intr_flag);
	ret = schedule_delayed_work(&hiusb_info_p->k3v2_otg_intr_work,0);
	spin_unlock_irqrestore(&(hiusb_info_p->intr_flag_lock), flags);
