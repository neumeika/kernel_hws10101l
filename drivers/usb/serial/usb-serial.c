...
/*
 * if you want add new modem for communication module,add VID&PID information here
 */
#define S10_VENDOR_ID     	0x12d1
#define HI6421_PRODUCT_ID      	0x1506
					...
					int num_ports, unsigned int *minor)
{
	...
	*minor = 0;
	...
	for (i = 0; i < SERIAL_TTY_MINORS; ++i) {
		...
		if (good_spot == 0)
			continue;
        //If the device is not a modem, ttyUSB number starting from 100
		if((S10_VENDOR_ID == serial->dev->descriptor.idVendor)
				&&(HI6421_PRODUCT_ID == serial->dev->descriptor.idProduct)){
			i = serial->interface->cur_altsetting->desc.bInterfaceNumber;			
		}
		else{
			i = i + 100;
		}
