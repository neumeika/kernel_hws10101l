...
static bool gser_flag = false;
static int gser_count = 0;

static void usb_gser_switch_delaywork(struct work_struct *work);
static DECLARE_DELAYED_WORK(usb_gser_switch_event, usb_gser_switch_delaywork);
static void usb_gser_switch_delaywork(struct work_struct *work)
{
	char *gser_switch[2]    = { "USB_STATE=GSERSWITCH", NULL };

	gser_flag = true;
	while (gser_flag && (10 > gser_count)) {
		pr_info("usb send uevent (%s) gser_count: %d.\n", gser_switch[0], gser_count);
		kobject_uevent_env(&_android_dev->dev->kobj, KOBJ_CHANGE, gser_switch);
		msleep(5000);
		gser_count++;
	}
	gser_count = 0;
...
void usb_gser_switch(void)
{
	schedule_delayed_work(&usb_gser_switch_event,msecs_to_jiffies(20));
	return;
}
EXPORT_SYMBOL( usb_gser_switch );
...
static unsigned int  acm_enable = 0;

static ssize_t acm_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
 {
 	return sprintf(buf, "%d", acm_enable);
 }

static ssize_t acm_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int iret = 0;

	printk(KERN_INFO "  acm_enable_store\n");

	if(NULL != buf) 
	{   
		iret = sscanf(buf,"%d", &acm_enable);
	}
...
static DEVICE_ATTR(acm_enable, S_IRUSR | S_IWUSR, acm_enable_show, acm_enable_store);

#define USB_OM_PORT_START  2
static int acm_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	...
	printk(KERN_INFO "acm_function_bind_config, instances:%d.\n",
        ...
        for (i = 0; i < config->instances; i++) {
                ...
                {
                    if(i>=USB_OM_PORT_START)
                    {   
                            ret = gser_om_bind_config(c, i);
                            if (ret) 
                            {
                                pr_err("Could not bind ser%u om config\n", i);
                                break;
                            }
                    }
                    else
...
static struct device_attribute *acm_function_attributes[] = { &dev_attr_instances, &dev_attr_acm_enable,NULL };
					...
					struct usb_composite_dev *cdev)
{
	...
	/* mass_storage as only one cdrom  */
	...
	while (b){
		...
		if(config->fsg.nluns >= FSG_MAX_LUNS_S10){
...
void send_usb_cdc_uevent(int uevent)
{
	struct android_dev *dev = _android_dev;
	kobject_uevent(&dev->dev->kobj,uevent);
}
EXPORT_SYMBOL_GPL(send_usb_cdc_uevent);
void usb_gadget_control(int on)
{
	if(on)
		enable_store(_android_dev->dev, NULL, "1", sizeof("1"));
	else
		enable_store(_android_dev->dev, NULL, "0", sizeof("0"));
}
EXPORT_SYMBOL_GPL(usb_gadget_control);
