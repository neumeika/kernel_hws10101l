...
static int __init hi6421_irq_probe(struct platform_device *pdev)
{
	...
	/* add by wufan w00163571 for protect null pointer */
	if(!pmu_irq_data->regulator_ocp_wq)
	{
		printk(KERN_ERR "%s: create work queue failed \n", __func__);
		ret = - ENOMEM;
		goto err_queue_request;
	}
...
//		goto err_gpio_free;
		/* begin add by wufan w00163571 */
		goto err_irq_request;
		/* end add by wufan w00163571 */
	}
...
err_irq_request:
	if(pmu_irq_data->regulator_ocp_wq)
	{
		destroy_workqueue(pmu_irq_data->regulator_ocp_wq);
		pmu_irq_data->regulator_ocp_wq = NULL;
	}
err_queue_request:
/* end add by wufan w00163571 */
err_gpio_free:
