...
char * cec_cmd_tostr(hdmi_cec_param* cec_cmd, char *str)
{
    int i = 0;
    int count = 0;
    count = sprintf(str, "CECCMD:%d %d %d %d ", cec_cmd->en_src_addr, cec_cmd->en_dst_addr, cec_cmd->opcode, cec_cmd->un_operand.raw_data.length);
    for (i = 0; i < sizeof(cec_cmd->un_operand.raw_data.data); i++) {
        count += sprintf(str+count, "%d ", cec_cmd->un_operand.raw_data.data[i]);
    }
    return str;
}
