...
#define MIN_FULL_OVC_2_ROTATE (450000)
...
u32 lsensor_h_latest = 0;
u32 lsensor_l_latest = 0;
static void sbl_workqueue(struct work_struct *ws)
{
	...
	if ((tmp_sbl & REG_SBL_EN) == REG_SBL_EN) {
		...
		if (res != ALold) {
            ...
            if ((lsensor_h != lsensor_h_latest) || (lsensor_l != lsensor_l_latest)) {
		...
		lsensor_h_latest = lsensor_h;
        lsensor_l_latest = lsensor_l;
	}
	up(&k3_fb_blank_sem);
...
static int k3_fb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (info->fix.smem_len < (k3_fb_line_length(var->xres, (var->bits_per_pixel >> 3))  * var->yres)) {
