...
extern struct platform_device *pdev_lcd;
#endif
...
#if defined(CONFIG_LCD_PANASONIC_VVX10F002A00) || defined(CONFIG_LCD_QIMEI_HJ101IA_01F_10)
			for (i = (sbl_old + SBL_STEP); i < k3fd->bl_level; i += SBL_STEP) {
                set_SBL_BKL_LEVEL_L_bkl_level_l(k3fd->edc_base, i);
				msleep(1);
            }
            sbl_old = k3fd->bl_level;
#endif            
...
extern u32 lsensor_l_latest;
extern u32 lsensor_h_latest;
int sbl_ctrl_resume(struct k3_fb_data_type *k3fd)
{
	...
	if ((K3_ENABLE == k3fd->sbl_enable) && ((tmp & REG_SBL_EN) != REG_SBL_EN)) {
        ...
        set_SBL_AMBIENT_LIGHT_L_ambient_light_l(k3fd->edc_base, lsensor_l_latest);
        set_SBL_AMBIENT_LIGHT_H_ambient_light_h(k3fd->edc_base, lsensor_h_latest);
		set_EDC_DISP_DPD_sbl_en(edc_base, K3_ENABLE);
		set_EDC_DISP_CTL_cfg_ok(edc_base, EDC_CFG_OK_YES);
