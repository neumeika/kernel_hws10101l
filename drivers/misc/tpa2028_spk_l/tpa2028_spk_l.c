...
enum PHONE_STATUS {
    PHONE_STATUS_IDLE    = 0,
    PHONE_STATUS_CALLING = 1,
};
unsigned int gPhoneStatus = PHONE_STATUS_IDLE;
#endif
...
static unsigned int gTpa2028EnFlag = 0;
static struct kobject *kobj = NULL;
//#ifdef CONFIG_BOOST_5V
//extern int request_5v_boost(int on,int bit_map);
//#endif
                            ...
                            unsigned long arg)
{
    ...
    case TPA2028_EN_PWR:
#ifdef CONFIG_BOOST_5V
        request_5v_boost(1,BOOST_5V_AUDIO);
#endif
        msleep(10);
        break;
    case TPA2028_DIS_PWR:
#ifdef CONFIG_BOOST_5V
        request_5v_boost(0,BOOST_5V_AUDIO);
#endif
        break;
    default:
...
#ifdef CONFIG_PM_SLEEP
static ssize_t phone_attr_show(struct kobject *kobj, struct kobj_attribute *attr,
        char *buf)
 {
     return sprintf(buf, "%d", gPhoneStatus);
 }

static ssize_t phone_attr_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count)
{
    int iret = 0;
    int readnum = -1;
    if(NULL != buf) {
        iret = sscanf(buf,"%d", &readnum);
    }

    if(-1 == iret) {
        printk(KERN_ERR "sscanf error\n");
        return iret;
    } 

    if (readnum == 0)
        gPhoneStatus = PHONE_STATUS_IDLE;
    else if(readnum == 1)
        gPhoneStatus = PHONE_STATUS_CALLING;
    else
        printk(KERN_ERR "get phone status error\n");
    return count;
}


static struct kobj_attribute phone_attribute =
     __ATTR(status, 0664, phone_attr_show, phone_attr_store);

static struct attribute* phone_attributes[] =
{
     &phone_attribute.attr,
     NULL
};

static struct attribute_group phone_defattr_group =
{
     .attrs = phone_attributes,
};
#endif
...
#ifdef CONFIG_PM_SLEEP
    kobj = kobject_create_and_add("phone_status", NULL);
    if (kobj == NULL)
    {
        return 0;
    }
    if (sysfs_create_group(kobj, &phone_defattr_group))
    {
        kobject_put(kobj);
    }
#endif
...
#ifdef CONFIG_PM_SLEEP
	sysfs_remove_group(kobj, &phone_defattr_group);
    kobject_put(kobj);
#endif
    kfree(platform_data);
    return 0;
...
{
    logi("%s", __FUNCTION__);
    if (gPhoneStatus == PHONE_STATUS_CALLING){
        return 0;
    }
    gpio_set_value(pdata->gpio_tpa2028_en, 0);
    gpio_direction_input(pdata->gpio_tpa2028_en);
    blockmux_set(tpa2028l_iomux_block, tpa2028l_block_config, LOWPOWER);
    request_5v_boost(0, BOOST_5V_AUDIO);
    return 0;
}
...
static int tpa2028_l_resume(struct i2c_client *client)
{
    ...
    request_5v_boost(1, BOOST_5V_AUDIO);
    blockmux_set(tpa2028l_iomux_block, tpa2028l_block_config, NORMAL);
