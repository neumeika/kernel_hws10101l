...
#define WAIT_COMPLT_TIMEOUT 1500
#else
...
static int ispv1_switch_config(camera_sensor *sensor, camera_state state)
{
	...
	if (state == STATE_PREVIEW) {
		...
		SETREG8(REG_ISP_SHARPNESS, sharpness); /* sharpness, y00215412 20120814 change 0x20->0x30 */
		#endif
		...
		SETREG8(REG_ISP_SHARPNESS, sharpness); /* sharpness, y00215412 2013-01-28 change from 0x10->0x0A */
		#endif
	}
	return 0;
}
	...
	camera_sensor *sensor, bool flash_on, camera_scene scene)
{
	...
	if (CAMERA_SCENE_ACTION == scene) {
		...
		if ((CAMERA_ISO_AUTO == this_ispdata->iso) && (1 == count)) {
			if (false == flash_on) {
				...
				/* added by y00215412 for ISP Denoise control in flash mode, more than 1band, denoise coff should larger  */
	...
	SETREG8(REG_ISP_CAPTURE_SKIP_FRAME, 1); /* fixed one frame by y00215412 for hdr 2013-01-29 */
