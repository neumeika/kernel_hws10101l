...
{
	di->tmr_ts_reg = (ENABLE_TMR_PIN | TMR_X_6)  | di->enable_low_chg;
...
k3_bq24161_charge_status(struct k3_bq24161_device_info *di)
{
	...
	if ((di->enable_ce == DISABLE_CE) || (read_reg[1] & BQ24161_VUSB_SUPPLY_FAULT)) {
	...
	if ((read_reg[0] & BQ24161_VUSB_FAULT) == BQ24161_CHARGE_DONE) {
...
k3_bq24161_low_current_charge(struct k3_bq24161_device_info *di)
{
	...
	if (is_k3_bq27510_battery_exist(g_battery_measure_by_bq27510_device)) {
		...
		if (battery_temperature < BQ24161_COLD_BATTERY_THRESHOLD) {
			...
			|| (battery_voltage < BQ24161_PRECONDITIONING_BATVOLT_THRESHOLD)) {
			...
			/*battery temp is between 00 and 50 degree,and battery voltage is more than 3.0V,normal charge*/
			if (di->charger_source == POWER_SUPPLY_TYPE_MAINS) {
				if (battery_voltage <= BQ24161_STARTUP_BATVOLT_THRESHOLD) {
			...
			/*battery voltage is more than 3.0V,normal charge*/
			...
			dev_warn(di->dev, "battery temp more than 50 degree,disable charging\n");
			di->enable_ce = DISABLE_CE;
...
static void k3_bq24161_charger_work(struct work_struct *work)
{
	...
	ret = k3_bq24161_read_byte(di, &read_reg, REG_BATTERY_VOLTAGE);
...
static int k3_bq24161_charger_suspend(struct i2c_client *client,
	pm_message_t state)
{
	printk("k3_bq24161_charger_suspend +\n");
	printk("k3_bq24161_charger_suspend -\n");
...
{
	printk("k3_bq24161_charger_resume +\n");
	printk("k3_bq24161_charger_resume -\n");
